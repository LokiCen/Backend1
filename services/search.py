...
def text_search(keywords):
    ...
    try:
        ...
        response = requests.post(
            'http://10.203.178.37:8000/searchfor/image',
        ...
        if code == 200:
            ...
            if not image_paths:  # 空结果处理
                return jsonify({
                    'code': 200,
                    'message': DEFAULT_RESPONSES['text_search'][0].format(keywords),
                    'data': []
                })
        elif code == 400:
            return jsonify({
                'code': 200,
                'message': DEFAULT_RESPONSES['text_search'][1],
                'data': []
            })
    except requests.exceptions.Timeout:
        return jsonify({
            'code': 200,
            'message': '请求超时，为您展示默认结果',
            'data': []
        })
    except Exception as e:
        return jsonify({
...
def image_search(image_file):
    ...
    try:
        ...
        if not content_list:  # 空结果处理
            return jsonify({
                'code': 200,
                'message': DEFAULT_RESPONSES['image_search'][0],
                'text_list': [{
                    'title': '默认建议',
                    'content': '这是系统根据您的图片生成的默认建议内容',
                    'source': '系统生成'
                }]
            })
    except requests.exceptions.Timeout:
        return jsonify({
            'code': 200,
            'message': DEFAULT_RESPONSES['image_search'][1],
            'text_list': [{
                'title': '超时提示',
                'content': '请求超时，这是系统提供的备选内容',
                'source': '系统缓存'
            }]
        })
    except Exception as e:
